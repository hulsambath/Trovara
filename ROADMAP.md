...
- [x] D1-4: Lock feature set / freeze scope (STATUS: DONE — 2026-07-12, scope freeze + non-goals in MASTER_PLAN.md § Scope Freeze, approved by Sambath. Day 1 complete)
...
- **D2-2 (editor formatting):** on-device Quill toolbar interaction needs a short manual pass — the editor isn't reachable via automated `enterText`.
- **D2-4 (AI chat):** live chat QA needs a keyed build (`--dart-define-from-file=configs/trovara_staging.json`) and human judgment of response quality.
- **D2-5 (Drive sync):** requires interactive Google sign-in on-device; silent-restore path is exercised crash-free in every E2E run.
