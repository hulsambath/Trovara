#!/usr/bin/env bash
# Stop hook: runs final quality checks before Copilot ends its turn.
...
# Exit codes:
#   0 = allow stop
#   2 = block stop (stderr sent to Copilot so it can fix and continue)
...
PROJECT_DIR="${COPILOT_PROJECT_DIR:-$(pwd)}"
cd "$PROJECT_DIR"
