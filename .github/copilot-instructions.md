...
## Copilot Guardrails (mirrors .claude hooks)

- **Never edit generated files**: `*.g.dart`, `*.freezed.dart`, `*.gr.dart`, `lib/gen/**`, `objectbox-model.json`. Edit the source and run `./scripts/build_runner.sh` when needed.
- **Format after edits**: for any non-generated `.dart` change, run `dart format --line-length 120 <file>`.
- **Model changes**: editing `lib/models/*.dart` requires `./scripts/build_runner.sh`.
- **Translations**: editing `assets/translations/*.json` requires an i18n parity check (same keys in `en.json` and `km.json`).
- **Core services/repositories**: editing `lib/core/services/**` or `lib/core/repository/**` requires `flutter test patrol_test`.
- **Before finishing**: run `flutter analyze` on the changed `.dart` files (or full analyze if unsure), and re-check i18n parity if translations changed.
- **File size limits**: keep files under limits (core hard limit 300 LOC; models 400; tests 500; `_view.dart` 40; `_content.dart` 200; widgets 150). Refactor before exceeding.
